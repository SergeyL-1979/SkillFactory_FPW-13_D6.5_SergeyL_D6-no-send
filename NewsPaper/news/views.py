...
# Выбираем категории из бар меню
# =============== По категориям детали =====================================
class NewsCategoryDetailView(LoginRequiredMixin, PermissionRequiredMixin, DetailView):
    model = Category
    ...
    def get_context_data(self, *args, **kwargs):
        ...
        context['all_category'] = Category.objects.filter()
        context['all_pc_dt'] = PostCategory.objects.filter()
        return context
...
class CategorySubscribe(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        send_mail(
            ...
            recipient_list=['test@gmail.com']  # здесь список получателей. Например, секретарь, сам врач и т. д.
        )
